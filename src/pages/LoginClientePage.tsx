import { Mail, Phone, ArrowRight, Loader2 } from "lucide-react";
import { Link, useNavigate } from "react-router-dom";
import { useState } from "react";
import { useGoogleLogin } from "@react-oauth/google";
import FacebookLogin from "@greatsumini/react-facebook-login";
import { useToast } from "@/hooks/use-toast";
import { useSocialLogin } from "@/hooks/useSocialLogin";
import { fetchApi } from "@/lib/api";
import { formatPhone } from "@/lib/utils";
import AuthLayout from "@/components/AuthLayout";
import PhoneCollectionModal from "@/components/PhoneCollectionModal";
import foodImage from "@/assets/food-login-cliente.jpg";

const FACEBOOK_APP_ID = import.meta.env.VITE_FACEBOOK_APP_ID as string;

const LoginClientePage = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const [email, setEmail] = useState("");
  const [telefone, setTelefone] = useState("");
  const [loading, setLoading] = useState<"email" | "sms" | null>(null);

  // Hook centralizado do login social (Gap 1, 2, 3, 5)
  const {
    state: socialState,
    errorMessage: socialError,
    provisionalData,
    handleGoogleToken,
    handleFacebookToken,
    handleCompleteRegistration,
  } = useSocialLogin();

  // Qualquer operação social ou OTP em andamento bloqueia toda a tela
  const isAnythingLoading = loading !== null || socialState === "loading";

  // ─── Handler: Login por e-mail OTP ───────────────────────────────
  const handleEmailLogin = async () => {
    if (!email) return;
    setLoading("email");
    try {
      const response = await fetchApi("/auth/request-otp", {
        method: "POST",
        body: JSON.stringify({ email }),
      });
      const data = await response.json();
      if (response.ok) {
        sessionStorage.setItem("login_email", email);
        sessionStorage.setItem("login_method", "email");
        navigate("/auth-login-cliente");
      } else {
        toast({
          title: "Erro",
          description: data.message || data.error || "Erro ao enviar código.",
          variant: "destructive",
        });
      }
    } catch {
      toast({
        title: "Erro de conexão",
        description: "Não foi possível conectar ao servidor.",
        variant: "destructive",
      });
    } finally {
      setLoading(null);
    }
  };

  // ─── Handler: Login por SMS OTP ──────────────────────────────────
  const handleSmsLogin = async () => {
    if (!telefone) return;
    setLoading("sms");
    try {
      const cleanTelefone = telefone.replace(/\D/g, "");
      const response = await fetchApi("/auth/request-otp", {
        method: "POST",
        body: JSON.stringify({ telefone: cleanTelefone }),
      });
      const data = await response.json();
      if (response.ok) {
        sessionStorage.setItem("login_telefone", cleanTelefone);
        sessionStorage.setItem("login_method", "sms");
        navigate("/auth-login-cliente");
      } else {
        toast({
          title: "Erro",
          description: data.message || data.error || "Erro ao enviar código.",
          variant: "destructive",
        });
      }
    } catch {
      toast({
        title: "Erro de conexão",
        description: "Não foi possível conectar ao servidor.",
        variant: "destructive",
      });
    } finally {
      setLoading(null);
    }
  };

  // ─── Handler: Google OAuth (Gap 1) ───────────────────────────────
  const loginWithGoogle = useGoogleLogin({
    onSuccess: (tokenResponse) => {
      handleGoogleToken(tokenResponse.access_token);
    },
    onError: () => {
      toast({
        title: "Login com Google cancelado",
        description: "Não foi possível autenticar com o Google.",
        variant: "destructive",
      });
    },
  });

  return (
    <AuthLayout
      backgroundImage={foodImage}
      panelTitle="Bem-vindo de volta!"
      panelSubtitle="Entre na sua conta para pedir suas refeições favoritas."
    >
      <h1 className="text-2xl font-extrabold text-foreground mb-2">Login do Cliente</h1>
      <p className="text-muted-foreground text-sm mb-8">
        Informe seu e-mail ou telefone para receber um código de acesso
      </p>

      <div className="space-y-4">
        {/* ── Campo e-mail ──────────────────────────────────────── */}
        <div className="relative">
          <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <input
            type="email"
            placeholder="Endereço de e-mail"
            aria-label="E-mail"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            disabled={isAnythingLoading}
            className="w-full h-12 pl-11 pr-4 rounded-xl bg-card border border-border text-sm font-medium text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all disabled:opacity-50"
          />
        </div>

        <button
          type="button"
          disabled={!email || isAnythingLoading}
          onClick={handleEmailLogin}
          className="w-full h-13 rounded-xl gradient-primary text-primary-foreground font-bold text-sm shadow-float hover:opacity-95 transition-opacity flex items-center justify-center gap-2 disabled:opacity-70"
        >
          {loading === "email" ? (
            <Loader2 size={16} className="animate-spin" />
          ) : (
            <>Receber código por e-mail <ArrowRight size={16} /></>
          )}
        </button>

        <div className="flex items-center gap-3 my-2">
          <div className="flex-1 h-px bg-border" />
          <span className="text-xs text-muted-foreground font-medium">ou</span>
          <div className="flex-1 h-px bg-border" />
        </div>

        {/* ── Campo telefone ────────────────────────────────────── */}
        <div className="relative">
          <Phone size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground" />
          <input
            type="tel"
            placeholder="(00) 00000-0000"
            aria-label="Telefone"
            value={telefone}
            onChange={(e) => setTelefone(formatPhone(e.target.value))}
            disabled={isAnythingLoading}
            className="w-full h-12 pl-11 pr-4 rounded-xl bg-card border border-border text-sm font-medium text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all disabled:opacity-50"
          />
        </div>

        <button
          type="button"
          disabled={!telefone || isAnythingLoading}
          onClick={handleSmsLogin}
          className="w-full h-13 rounded-xl bg-card border border-border text-foreground font-bold text-sm hover:bg-muted transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
        >
          {loading === "sms" ? (
            <Loader2 size={16} className="animate-spin" />
          ) : (
            <>Receber código por SMS <ArrowRight size={16} /></>
          )}
        </button>

        <div className="flex items-center gap-3 my-2">
          <div className="flex-1 h-px bg-border" />
          <span className="text-xs text-muted-foreground font-medium">ou entre com</span>
          <div className="flex-1 h-px bg-border" />
        </div>

        {/* ── Botões Sociais (Gap 1 + Gap 2) ───────────────────── */}
        <div className="flex gap-3">
          {/* Botão Google — integrado com @react-oauth/google */}
          <button
            id="btn-social-google"
            type="button"
            disabled={isAnythingLoading}
            onClick={() => loginWithGoogle()}
            className="flex-1 h-12 rounded-xl bg-card border border-border text-foreground font-semibold text-sm hover:bg-muted transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
          >
            {socialState === "loading" ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
              <>
                <svg width="18" height="18" viewBox="0 0 24 24">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
                </svg>
                Gmail
              </>
            )}
          </button>

          {/* Botão Facebook — integrado com @greatsumini/react-facebook-login */}
          <FacebookLogin
            appId={FACEBOOK_APP_ID}
            onSuccess={(response) => handleFacebookToken(response.accessToken)}
            onFail={() => {
              toast({
                title: "Login com Facebook cancelado",
                description: "Não foi possível autenticar com o Facebook.",
                variant: "destructive",
              });
            }}
            render={({ onClick }) => (
              <button
                id="btn-social-facebook"
                type="button"
                disabled={isAnythingLoading}
                onClick={onClick}
                className="flex-1 h-12 rounded-xl bg-card border border-border text-foreground font-semibold text-sm hover:bg-muted transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
              >
                {socialState === "loading" ? (
                  <Loader2 size={16} className="animate-spin" />
                ) : (
                  <>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="#1877F2">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                    Facebook
                  </>
                )}
              </button>
            )}
          />
        </div>

        {/* ── Tratamento de erro social (Gap 5) ────────────────── */}
        {socialState === "error" && socialError && (
          <p className="text-red-500 text-sm text-center mt-2">{socialError}</p>
        )}

        <p className="text-center text-sm text-muted-foreground mt-4">
          Não tem uma conta?{" "}
          <Link to="/email-cadastro-cliente" className="text-primary font-semibold hover:underline">
            Cadastre-se
          </Link>
        </p>
      </div>

      {/* ── Modal de coleta de telefone (Gap 3) — renderiza até success/error ── */}
      {(socialState === "require_phone" || (isAnythingLoading && provisionalData)) && provisionalData && (
        <PhoneCollectionModal
          nomeCompleto={provisionalData.nome_completo}
          email={provisionalData.email}
          onConfirm={handleCompleteRegistration}
          isLoading={isAnythingLoading}
          errorMessage={socialError}
        />
      )}
    </AuthLayout>
  );
};

export default LoginClientePage;
